...
export type DragItemData = DragTaskData | DragListData;

export type ACTIONTYPE =
  | { type: typeof DRAG_START; payload: DragItemData }
  | { type: typeof DRAG_END; payload: { activeId: string | null; overId: string | null } }
  | {
      ...
      payload: {
        ...
        overRect: DOMRect | null | undefined;
      };
    }
...
export const kanbanReducer = (state: KanbanState, action: ACTIONTYPE): KanbanState => {
  switch (action.type) {
    case DRAG_START: {
      if (action.payload.type === 'task') return { ...state, draggedTask: action.payload.task };
      if (action.payload.type === 'list') return { ...state, draggedList: action.payload.list };
