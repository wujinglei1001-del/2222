...
const WebApps = ({ data }: { data: WebApp[] }) => {
  ...
  return (
    ...
    >
      ...
      />
      <Stack
        sx={{ alignItems: 'center', px: 3, position: 'relative', zIndex: 3, textAlign: 'center' }}
      >
        ...
        <RevealOnScroll delay={0.4}>
          <Stack direction="row" sx={{ gap: 0.5, mb: 3, mt: 4 }}>
            <NeutralButton
              ref={prevRef}
              size="large"
      ...
      <Stack direction="row" sx={{ gap: 0.5, mt: 2, justifyContent: 'center' }}>
        {data.map((_, i) => (
          <ButtonBase
            key={i}
            onClick={() => slideTo(i)}
