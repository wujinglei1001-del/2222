...
const FilterDrawerContent = ({ handleClose }: FilterDrawerContentProps) => {
  return (
    <Box
      component="aside"
      sx={{
...
const FilterFieldset = ({ label, options }: FilterFieldsetProps) => {
  const [state, setState] = useState(
    options.reduce(
      (acc: { [key: string]: boolean }, option: { label: string; value: boolean }) => ({
        ...acc,
...
const FilterRangeField = ({ label, range, valueText }: FilterRangeFieldProps) => {
  const [value, setValue] = useState<number[]>(range);

  const handleChange = (_: Event, newValue: number[]) => setValue(newValue);
  return (
    <FormControl component="fieldset" variant="standard" sx={{ px: 2 }}>
      <StyledFormLabel>{label}</StyledFormLabel>
      <FormGroup sx={{ pl: 2 }}>
