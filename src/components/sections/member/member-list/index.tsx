...
import { Box, ChipProps, styled } from '@mui/material';
import { MemberData, membersListData } from 'data/member/member-list';
...
export type ListTab = 'list' | 'grid' | 'org-chart';

interface MemberListMainProps {
  toggleDrawer: () => void;
  isDrawerOpen: boolean;
}

const MemberListMain = ({ toggleDrawer, isDrawerOpen }: MemberListMainProps) => {
  const [tab, setTab] = useState<ListTab>('list');
  const handleChange = (_event: SyntheticEvent, view: ListTab) => setTab(view);
  return (
...
export const getStatusChipColor = (status: MemberData['status']): ChipProps['color'] => {
  switch (status) {
    case 'Active':
      return 'success';
    case 'Resigned':
