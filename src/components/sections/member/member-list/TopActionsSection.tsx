...
interface TopActionsSectionProps {
  tab: ListTab;
  handleChange: (_: SyntheticEvent, view: ListTab) => void;
  toggleDrawer: () => void;
  isDrawerOpen: boolean;
}

const TopActionsSection = ({ tab, handleChange, toggleDrawer }: TopActionsSectionProps) => {
  ...
  return (
    <Stack
      direction={{ md: 'row' }}
      sx={{ gap: 1.5, width: 1, justifyContent: 'space-between', alignItems: { lg: 'center' } }}
    >
      <SearchTextField
