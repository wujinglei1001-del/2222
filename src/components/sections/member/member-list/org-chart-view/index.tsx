...
import { Box, BoxProps, Button, Paper, Stack, styled } from '@mui/material';
import { OrgMemberNode } from 'data/member/member-tree';
...
interface MembersOrgChartViewProps {
  data: OrgMemberNode;
}

const MembersOrgChartView = ({ data }: MembersOrgChartViewProps) => {
...
interface ZoomButtonsProps extends BoxProps {
  zoom: number;
  zoomIn: () => void;
  zoomOut: () => void;
}

const ZoomButtons = ({ zoom, zoomIn, zoomOut, sx, ...rest }: ZoomButtonsProps) => {
  return (
    <Box
      sx={{
        position: 'sticky',
